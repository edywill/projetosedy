...
var url = "consValorTra.php?valor="+valor+"&tipo="+tipo;
...
document.getElementById('vldia').value= float2moeda(moeda2float(myarr[0]));
...
echo "<h3>Atualizar Solicitação de Transporte</h3>";
$sqlDadosTra=mysql_query("SELECT convtra.*,convtra.id AS idtra FROM convtra WHERE convtra.id='".$idItem."'") or die(mysql_error());
$arrayDadosTra=mysql_fetch_array($sqlDadosTra);
$local=$arrayDadosTra['local'];
$tipo=$arrayDadosTra['tipo'];
$abrg='';
$cidadeHos='';
if($arrayDadosTra['abrg']=='nac'){
	$abrg='Nacional';
	$cidadeHos=utf8_encode($arrayDadosTra['local']);
	}elseif($arrayDadosTra['abrg']=='inter'){
		$abrg='Internacional';
		$cidadeHos=utf8_encode($arrayDadosTra['local']);
		}else{

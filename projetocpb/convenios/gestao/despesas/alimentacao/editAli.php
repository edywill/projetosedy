...
echo "<h3>Atualizar Solicitação de Alimentação</h3>";
$sqlDadosAli=mysql_query("SELECT convali.*,convali.id AS idali FROM convali WHERE convali.id='".$idItem."'") or die(mysql_error());
$arrayDadosAli=mysql_fetch_array($sqlDadosAli);

$dtfim=$arrayDadosAli['dtfim'];
...
$jant='';
if($arrayDadosAli['jan']==1){
	$jant='checked';
	}

$abrg='';
$cidadeHos='';
if($arrayDadosAli['abrg']=='nac'){
	$abrg='Nacional';
	$cidadeHos=utf8_encode($arrayDadosAli['local']);
	}elseif($arrayDadosAli['abrg']=='inter'){
		$abrg='Internacional';
		$cidadeHos=utf8_encode($arrayDadosAli['local']);
		}else{
...
$titButton='Atualizar';
echo "<form action='updateAli.php' name='formProjConv' method='post'>";
echo "<input type='hidden' name='idproj' value='".$idProj."'/>
<input type='hidden' name='idAli' value='".$idItem."'/>

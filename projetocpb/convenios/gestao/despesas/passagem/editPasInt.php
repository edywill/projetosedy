...
$sqlDadosPas=mysql_query("SELECT convpas.*,convpas.trecho AS trechoP,convpasintreferencia.*,convpasintreferencia.trecho AS trechoRef FROM convpas,convpasintreferencia WHERE convpas.id='".$_POST['idPas']."' AND convpas.trecho=convpasintreferencia.id") or die(mysql_error());
$arrayDadosPas=mysql_fetch_array($sqlDadosPas);
$trecho=utf8_encode($arrayDadosPas['trechoRef']);
$idRef=$arrayDadosPas['trechoP'];
$abrg='Nacional';
if($arrayDadosPas['abrg']=='int'){
	$abrg='Internacional';
	}
$qtd=$arrayDadosPas['qtd'];
$dtinicio=$arrayDadosPas['dtin'];
$dtfim=$arrayDadosPas['dtfim'];
$vlunit=$arrayDadosPas['valor'];
...
<input type='hidden' name='idPas' value='".$_POST['idPas']."'/>

...
echo "<h3>Atualizar RH ".$_POST['titMod']."</h3>";
$sqlDadosRh=mysql_query("SELECT convrh.*,convrhreferencia.funcao FROM convrh,convrhreferencia WHERE convrh.id='".$idItem."' AND convrh.idcargo=convrhreferencia.id") or die(mysql_error());
$arrayDadosRh=mysql_fetch_array($sqlDadosRh);
$idcargo=$arrayDadosRh['idcargo'];
$nomecargo=$arrayDadosRh['funcao'];
$qtdpes=$arrayDadosRh['qtdpes'];
$qtdtem=$arrayDadosRh['qtdtem'];
$um=$arrayDadosRh['um'];
$vlunit=$arrayDadosRh['vlunit'];
$tributos=$arrayDadosRh['tributos'];
$total=$arrayDadosRh['total'];
$tcont=$arrayDadosRh['tcont'];
$titButton='Atualizar';
echo "<form action='updateRh.php' name='formProjConv' method='post'>";
echo "<input type='hidden' name='idproj' value='".$idProj."'/>
<input type='hidden' name='idRh' value='".$idItem."'/>

...
<body>
<div id="box3">
<h2>SOLICITA&Ccedil;&Otilde;ES COM PRAZO INFERIOR</h2>
<strong>CIs Pendentes de An&aacute;lise:</strong>
<?php 
//include "function.php";
echo "<div id='funcoes' style='display: none;'>";
$usuarioSolF=$_GET['usuario'];
require ("conexaomysql.php");
$pegarDadosUser=mysql_query("SELECT id,cigam FROM usuarios WHERE nome='".$usuarioSolF."'");
$arrayDados=mysql_fetch_array($pegarDadosUser);

include "functionPrazos.php";
if(!empty($arrayDados['cigam'])){
//Busco o setor do gestor
$setorGestor=buscaSetorPrazos($arrayDados['cigam']);
//Busco as variáveis de controle com base no setor do gestor
$controles=buscaControlesPrazos($setorGestor['cd_setor']);
//Crio as variáveis
$ctrlInferiorGestor=$controles['ctrl_prz_inferior'];
$rejControle=$controles['ctrl_prz_rejeitado'];
$rejSitSolicitacao=$controles['sit_solicitacao'];
$rejSitItem=$controles['situac_item_sol'];
echo "</div>";
//Listo as CIs que encontram-se pendentes
listaCiPrazosGestor($ctrlInferiorGestor,$arrayDados['cigam'],$setorGestor['cd_setor']);
}
?>
<br />
</div>

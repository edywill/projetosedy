...
function montaCabecalho($funcionario,$mes, $ano){
	...
	$mesAtual=$ano."/01/".$mes." 00:00:00.000";
			$cargoCont=$array_resultado["DESCRICAO202"];
			$sqlContCargo=odbc_exec($conCab,"Select
  RHCONTRATOS.DATAULTALTCARGO,
  RHALTERACOESCONTRATO.CODIGOANTERIOR,
...
From
  ...
  RHALTERACOESCONTRATO with (nolock) On RHALTERACOESCONTRATO.UNIDADE = RHCONTRATOS.UNIDADE And
    RHALTERACOESCONTRATO.CONTRATO = RHCONTRATOS.CONTRATO Inner Join
  RHCARGOS with (nolock) On RHCONTRATOS.CARGO = RHCARGOS.CARGO
Where
  RHPESSOAS.PESSOA = '".$array_resultado['PESSOA']."'
  AND RHALTERACOESCONTRATO.CODIGOANTERIOR <>'' 
  AND RHALTERACOESCONTRATO.CODIGOATUAL <> ''
  AND RHALTERACOESCONTRATO.DATAHORAALTERACAO > '".$mesAtual."'") or die("<p>".odbc_errormsg());
			while($arrayContCargo=odbc_fetch_array($sqlContCargo)){
			if($arrayContCargo['DATAHORAALTERACAO']>$mesAtual){
			$sqlCargoCont=odbc_exec($conCab,"Select
  RHCARGOS.DESCRICAO20,
  RHCARGOS.CARGO
From
  RHCARGOS   
  where CARGO='".$arrayContCargo['CODIGOANTERIOR']."'");
			$arrayNomeCargo=odbc_fetch_array($sqlCargoCont);
			if(!empty($arrayNomeCargo['DESCRICAO20'])){
				$cargoCont=$arrayNomeCargo["DESCRICAO20"];
				}	
			}
		}
...
function montaRodape($nome){
 ...
 function montaCabecalhoContCheq($mesCh,$anoCh,$nome_funcCh){
     ...
     $SQLCht = "Select
	  ...
	  $mesAtual=$anoCh."/01/".$mesCh." 00:00:00.000";
...
From
  ...
  RHCARGOS with (nolock) On RHCONTRATOS.CARGO = RHCARGOS.CARGO
Where
  RHPESSOAS.PESSOA = '".$array_resultadoCht['PESSOA']."'
  AND RHALTERACOESCONTRATO.CODIGOANTERIOR <>'' 
  AND RHALTERACOESCONTRATO.CODIGOATUAL <> ''
  AND RHALTERACOESCONTRATO.DATAHORAALTERACAO > '".$mesAtual."'") or die("<p>".odbc_errormsg());
			while($arrayContCargo=odbc_fetch_array($sqlContCargo)){
...
if($tipoCh13==51){
	...
	$array_resultadoChC13 = odbc_fetch_array($resChC13);
    $array_resultadoChR13 = odbc_fetch_array($resChR13);
        	$valor131=$array_resultadoChC13["VALOR"];
			echo"    <tr>"; 
	...
	echo "  ".$banco13."";
	echo " &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ";
	echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Agencia: ".$agencia13."-".$digitoAg13."</td>";
	echo "    <td width='170' align='right'><strong>L i q u i d o<br />".number_format($valor131,2, ',', '.')."</strong></td>";
	echo "  </tr>";
	echo "</table>";
	echo "<table width='845' border='1' cellpadding='0' cellspacing='0'>";
	echo "  <tr>";
...
function montaGradePendente13($usuarioGradeP13){
	 $rs13 = mysql_query("SELECT * FROM sol13  WHERE funcionario='".$usuarioGradeP13."' and status=1");
...
function montaGradeAprovadas13($usuarioGradeAp13){
	 $rs113 = mysql_query("SELECT * FROM sol13  WHERE funcionario='".$usuarioGradeAp13."' and status=2");
...
function montaGradeRecusadas13($usuarioGradeRe13){
	 $rs213 = mysql_query("SELECT * FROM sol13  WHERE funcionario='".$usuarioGradeRe13."' and status=3");
...
function excluirUserIntranet($idUserIntranet){
	...
	if ($deleteSqlUserIntranet) {
       ...
       header('Localização: listaUsuarios.php');
	          ...
	          ?>
       <script type="text/javascript">
       alert("Usuario não excluido. Tente novamente!");
       history.back();
       </script>
       <?php
       header('Localização: listaUsuarios.php');
...
function imprimirFerias13($idFerias13){
  ...
  <tr>
    <td><p align='center'><img src='imagens/logoDocumento.png' alt='' width='95' height='120' /> <br />
      ...
      <p align='center'>&nbsp;</p>
        ...
        <br />
        <strong>De</strong>: ".$objImprimir13->funcionario."</p>
  ...
  <tr>
    <td><p><br>Tendo em vista dificuldades financeiras no momento, venho solicitar que me seja adiantado a 1ª parecela do 13º salário do corrente ano.</p>
      <p align='center'>Solicitação aprovada por:<strong> ".$objImprimir13->gestor."</strong> . </p>
  ...
  <strong>".$objImprimir13->funcionario."</strong></p>
     <br>
	 <br>
	 <p align='center'> APROVADO POR: <strong>".$objImprimir13->gestor."</strong></p>
  </td>
  </tr>
  <tr>
...
function imprimirFerias($idFerias){
	...
	require('conectsqlserver.php');
	   ...
	   $imprimirSQL = mysql_query("SELECT * FROM solferias  WHERE solferias.id='".$idFerias."'");
		...
		$dataFerias=odbc_exec($conCab,"Select
...
Where
  RHPESSOAS.NOMECOMPLETO = '".$objImprimir->funcionario."'");
...
$dtInicioFerias=($dtFeriasCompleto[0]+1)." / ".$dtFeriasCompleto[1]." / ".($dtFeriasCompleto[2]-1);
$dtFimFerias=($dtFeriasCompleto[0])." / ".$dtFeriasCompleto[1]." / ".($dtFeriasCompleto[2]);
  ...
  <strong>".$objImprimir->funcionario."</strong></p>
     <br>
	 <br>
	  <p align='center'> APROVADO POR:  <strong>".$objImprimir->gestor."</strong></p>
  </td>
  </tr>
  <tr>
    <td><img src='imagens/rodapeDocumento.png' width='703' height='77' /></td>
...
function listaSolicitac13($userGestor13){
	 $rsGest13 = mysql_query("SELECT * FROM sol13  WHERE status=1 and gestor='".$userGestor13."' ");
...
echo "<form action='http://www.cpb.org.br/intranetcpb/enviaEmail.php' method='post' name='form5.id_CI' > <table width='100%' align='center'><tr><td>
<div id='outro' style='display: none;'>
<input name='user_ci' id='user_ci' value='".$resIdIntranet['nome']."' size='40' type='hidden' />
<input name='emailRem' id='emailRem' value='".$resIdIntranet['email']."' size='40' type='hidden' />
<input name='id_ci' id='id_ci' value='".$ciUpdate."' size='40' type='hidden' />
<input name='desc_ci' id='desc_ci' value='".$descricaoCiUpdate."' size='40' type='hidden' />
<input name='dest_ci' id='email_ci' value='".$resIdIntranetEmail['nome']."' size='40' type='hidden' />
<input name='email_ci' id='email_ci' value='".$resIdIntranetEmail['email']."' size='40' type='hidden' />
<input name='controleAnterior' id='controleAnterior' value='".$controleCIEmail."' size='40' type='hidden' />
<input name='descControleAnterior' id='descControleAnterior' value='".rtrim($descContCIEmail)."' size='40' type='hidden' />
<input name='controleNovo' id='controleNovo' value='".$controleNovoCiUpdate."' size='40' type='hidden' />
<input name='descControleNovo' id='descControleNovo' value='".rtrim($descContCINovo)."' size='40' type='hidden' />
<input name='retorno' id='descControleNovo' value='".rtrim($pgRetornoUp)."' size='40' type='hidden' />
</div>
<h6>SOLICITA&Ccedil;&Atilde;O ATUALIZADA COM SUCESSO.<br> PARA CONTINUAR CLIQUE NO BOT&Atilde;O ABAIXO.</h6></td></tr><tr><td>
<input name='enviar7' class='button' type='submit' value='CONTINUAR' /></td></tr></table></form>";
            }
			}
			}
			}
...
From
  COSOLICI Inner Join
  COISOLIC On COSOLICI.Solicitacao = COISOLIC.Cd_solicitacao Inner Join
...
while($objresConsultaEmails = odbc_fetch_object($resConsultaEmails)){
	if($listaEmail==''){
		$listaEmail=rtrim($objresConsultaEmails->email);
		$emailAnt=rtrim($objresConsultaEmails->email);
	}else{
		if(rtrim($objresConsultaEmails->email)<>$emailAnt){
			$listaEmail=$listaEmail.";".rtrim($objresConsultaEmails->email);
			}
		$emailAnt=rtrim($objresConsultaEmails->email);
		}
    }
...
echo "</div>";
//$descContCIItemEmailNovo=mb_convert_encoding($arrayConsContrCIAnt['descricao'],"UTF-8","ISO-8859-1");
echo "<form action='http://www.cpb.org.br/intranetcpb/enviaEmail.php' method='post' name='form5.id_CI' > 
...
<input name='id_ci' id='id_ci' value='".$ciUpdate."' size='40' type='hidden' />
<input name='desc_ci' id='desc_ci' value='".$descricaoCiUpdate."' size='40' type='hidden' />
<input name='email_ci' id='email_ci' value='".$listaEmail."' size='40' type='hidden' />
<input name='controleAnterior' id='controleAnterior' value='".$controleCIEmailAP."' size='40' type='hidden' />
<input name='descControleAnterior' id='descControleAnterior' value='".rtrim($descContCIEmailAP)."' size='40' type='hidden' />
<input name='controleNovo' id='controleNovo' value='".$controleCIEmailAPNovo."' size='40' type='hidden' />
<input name='descControleNovo' id='descControleNovo' value='".rtrim($descContCIEmailAPNovo)."' size='40' type='hidden' />
<input name='retorno' id='descControleNovo' value='".rtrim($pgRetornoUp)."' size='40' type='hidden' />

<h6>SOLICITA&Ccedil;&Atilde;O ATUALIZADA COM SUCESSO.<br> PARA CONTINUAR CLIQUE NO BOT&Atilde;O ABAIXO.</h6></td></tr><tr><td><input name='enviar7' class='button' type='submit' value='CONTINUAR' /></td></tr></table></form>";
					}
				  }
			}
	}
...
From
  ...
  ORDER BY COSOLICI.Solicitacao DESC";
       ...
       <?php
		...
		<tr><th >Controle</th><td><select name='controle'><option selected='selected'>Escolha</option><option value='16'> 16 </option><option value='EP'> EP </option></select><input name='enviar5' class='buttonVerde' type='submit' value='Alterar' /></form>
		...
		<td colspan='2'><form action='imprimeCi.php' method='post' name='form4.id_CIImprimir' ><input name='id_ciImpressao' id='id_ciImpressao' value='".$objCiV->Solicitacao."' size='40' type='hidden' /><input name='enviar7' class='buttonVerde' type='submit' value='Visualizar CI' /></form></td></tr><tr><td colspan='8' height='25'></td></tr>";}
       ...
       <?php
		...
		<tr><th width='40'>Aprova/Reprova</th><td>
		 <select name='controle'>
			...
			<input name='enviar5' class='buttonVerde' type='submit' value='Alterar' /></form>
		...
		<td colspan='2'><form action='imprimeCi.php' method='post' name='form4.id_CIImprimir' ><input name='id_ciImpressao' id='id_ciImpressao' value='".$objCiV->Solicitacao."' size='40' type='hidden' />
       ...
       <?php
			...
			echo "<form action='atualizaCi.php' method='post' name='form4.id_CI' onsubmit=\"this.elements['enviar5''].disabled=true;\"> 
					<tr><td bgcolor='#658BF3' colspan='7'><center><strong> Solicita&ccedil;&atilde;o N&ordm; 
							...
							<tr><th width='40'>Aprova/Reprova</th><td><input name='enviar5' class='buttonVerde' type='submit' value='Aprovar CI' /></form></td>
						...
						<form action='imprimeCi.php' method='post' name='form4.id_CIImprimir' >
		  ...
		  $arrayImpCi = odbc_fetch_array($resImp2CI);
		     	$SQLItemEmbarqueImp = "select Embarque_pedido, DATA, Historico
					...
					$SQLItemSoliciImp = "Select
										  ...
										  IT.Sequencia_ordem
										From
										  COSOLICI SOL with (nolock) Inner Join
				    ...
				    echo "<H2>SOLICITA&Ccedil;&Atilde;O INTERNA</H2>";
		   ...
		   AND htl.sequencia='".$objImpCi->Sequencia."'";
			...
			if(odbc_num_rows($resItemHotelImp)>0){
								...
								while($objItemHotelImp = odbc_fetch_object($resItemHotelImp)){
									...
									echo "<tr><td>".$objItemHotelImp->num."</td><td>".$objItemHotelImp->reserva."</td><td>".$objItemHotelImp->Nome_completo."</td><td>".$cargoImpCiHotel."</td><td>".date('d/m/Y',strtotime($objItemHotelImp->dt_entrada))."</td><td>".date('d/m/Y',strtotime($objItemHotelImp->dt_saida))."</td></tr>";
									}
								echo "</div></table>";
								}
							}
					...
					$resItemRPAImp = odbc_exec($conCab, $SQLItemRPAImp);
							if(odbc_num_rows($resItemRPAImp)>0){
								...
								while($objItemRPAImp = odbc_fetch_object($resItemRPAImp)){
									...
									echo "<tr><td>".$objItemRPAImp->Profissional."</td><td>".$cargoImpCiRpa."</td><td>".date('d/m/Y',strtotime($objItemRPAImp->dt_inicio))."</td><td>".date('d/m/Y',strtotime($objItemRPAImp->dt_fim))."</td><td>R$ ".number_format($objItemRPAImp->valor, 2, ',', '.')."</td></tr>";
									}
								echo "</div></table>";
								}
							}
					...
					$resItemDiariaImp = odbc_exec($conCab, $SQLItemDiariaImp);
							if(odbc_num_rows($resItemDiariaImp)>0){
								echo "<div id='tabela'><table border='1'><tr><td><strong>PROFISSIONAL</strong></td><td><strong>CARGO</strong></td><td><strong>IN&Iacute;CIO</strong></td><td><strong>T&Eacute;RMINO</strong></td><td><strong>VALOR</strong></td></tr>";
								while($objItemDiariaImp = odbc_fetch_object($resItemDiariaImp)){
									...
									echo "<tr><td>".$objItemDiariaImp->Profissional."</td><td>".$cargoImpCiDiaria."</td><td>".date('d/m/Y',strtotime($objItemDiariaImp->dt_inicio))."</td><td>".date('d/m/Y',strtotime($objItemDiariaImp->dt_termino))."</td><td>R$ ".number_format($objItemDiariaImp->valor, 2, ',', '.')."</td></tr>";
									}
								echo "</div></table>";
								}
							} 
						}
		$sqlSolicitado="select top 1 
 case 
          when substring(historico,patindex('%de%',historico)+3,2) between '03' and '05' then 'SOLICITADO POR:'
          when substring(historico,patindex('%de%',historico)+3,2) in ('14','15','17','18','19') then 'APROVADO ORÇAMENTO:'
          when substring(historico,patindex('%de%',historico)+3,2) = 'AP' then 'APROVADO GESTOR:'
          when substring(historico,patindex('%de%',historico)+3,2) = '16' then 'APROVADO SICONV:'
          when substring(historico,patindex('%de%',historico)+3,2) = 'EP' then 'APROVADO PRESIDENCIA:'
          else 'DESCONHECIDO'
       end tipo,
       rtrim((select nome from GEUSUARI usu (nolock) where usu.cd_usuario = aco.usuario)) + ' em '+
       convert(varchar,aco.data,103) + ' &agrave;s ' +
       SUBSTRING(aco.hora,1,2) +':'+SUBSTRING(aco.hora,3,2) Descricao,
        aco.data,
        aco.hora
from GEACOMP aco (nolock)
where tipo_acompanham = 'R'
and sequencia_item = 0
and campo26 = 'N'
and campo39 = 1
and campo40 = 1
and rtrim(ltrim(embarque_pedido)) = '".$numeroCiImpressao."'
--desconsidera trocas regredidas
and substring(historico,patindex('%de%',historico)+3,2) <= substring(historico,patindex('%para%',historico)+5,2)
and substring(historico,patindex('%de%',historico)+3,2) between '03' and '05'
order by
DATA desc,
hora desc,
 case 
            when substring(historico,patindex('%de%',historico)+3,2) between '03' and '05' then 1
            when substring(historico,patindex('%de%',historico)+5,2) in ('14','15','17','18','19') then 2
            when substring(historico,patindex('%de%',historico)+5,2) = 'AP' then 3
            when substring(historico,patindex('%de%',historico)+5,2) = '16' then 4
            when substring(historico,patindex('%de%',historico)+5,2) = 'EP' then 5
            else 6
         end ";
		 $resSolicitado = odbc_exec($conCab, $sqlSolicitado);
		 $arraySolicitado=odbc_fetch_array($resSolicitado);
		 $sqlOrcamento="select top 1
case 
          when substring(historico,patindex('%de%',historico)+3,2) in ('14','15','17','18','19') then 'APROVADO ORÇAMENTO:'
          else 'DESCONHECIDO'
       end tipo,
...
and rtrim(ltrim(embarque_pedido)) = '".$numeroCiImpressao."'
--desconsidera trocas regredidas
and substring(historico,patindex('%de%',historico)+3,2) in('14','15','17','18','19')
order by 
DATA desc,
Hora desc,
case 
            when substring(historico,patindex('%de%',historico)+3,2) between '03' and '05' then 1
            when substring(historico,patindex('%de%',historico)+3,2) in ('14','15','17','18','19') then 2
            when substring(historico,patindex('%de%',historico)+3,2) = 'AP' then 3
            when substring(historico,patindex('%de%',historico)+3,2) = '16' then 4
            when substring(historico,patindex('%de%',historico)+3,2) = 'EP' then 5
            else 6
         end ";
		 $resOrcamento = odbc_exec($conCab, $sqlOrcamento);
		 $arrayOrcamento=odbc_fetch_array($resOrcamento);
		 $sqlApGestor="select top 1
case 
          when substring(historico,patindex('%de%',historico)+3,2) = 'AP' then 'APROVADO GESTOR:'
          else 'DESCONHECIDO'
       end tipo,
...
and rtrim(ltrim(embarque_pedido)) = '".$numeroCiImpressao."'
--desconsidera trocas regredidas
and substring(historico,patindex('%de%',historico)+3,2) = 'AP'
order by
DATA desc,
Hora desc,
 case 
		 ...
		 $resApGestor = odbc_exec($conCab, $sqlApGestor);
		 $arrayApGestor=odbc_fetch_array($resApGestor);
		 $sqlSICONV="select top 1
case 
          when substring(historico,patindex('%de%',historico)+3,2) = '16' then 'APROVADO SICONV:'
          else 'DESCONHECIDO'
       end tipo,
...
and rtrim(ltrim(embarque_pedido)) = '".$numeroCiImpressao."'
--desconsidera trocas regredidas
and substring(historico,patindex('%de%',historico)+3,2) = '16'
order by
data desc,
Hora desc,
 case 
		 ...
		 $resSICONV = odbc_exec($conCab, $sqlSICONV);
		 $arraySICONV=odbc_fetch_array($resSICONV);
		 $sqlApPres="select top 1
case 
          when substring(historico,patindex('%de%',historico)+3,2) = 'EP' then 'APROVADO PRESIDENCIA:'
          else 'DESCONHECIDO'
       end tipo,
...
and rtrim(ltrim(embarque_pedido)) = '".$numeroCiImpressao."'
--desconsidera trocas regredidas
and substring(historico,patindex('%de%',historico)+3,2) = 'EP'
order by
data desc,
hora desc,
 case 
		 ...
		 $resApPres = odbc_exec($conCab, $sqlApPres);
		 $arrayApPres=odbc_fetch_array($resApPres);
		//echo "</div></div></div></div><br><br><table width='100%' border='0'><tr><td width='20%' border=0></td><td align='center'>
		echo "</div></div></div></div><br><br><div id='tabela'><table border='1'>";
		if(odbc_num_rows($resSolicitado)>0){
		      echo "<tr><td><strong>Solicitado por:</strong></td><td>".$arraySolicitado['Descricao']."</td></tr>";
		}
		if(odbc_num_rows($resApGestor)>0){
		      echo "<tr><td><strong>Aprovado Gestor:</strong></td><td>".$arrayApGestor['Descricao']."</td></tr>";
		}
		if(odbc_num_rows($resOrcamento)>0){
		      echo "<tr><td><strong>Aprovado Or&ccedil;amento:</strong></td><td>".$arrayOrcamento['Descricao']."</td></tr>";
		}
		if(odbc_num_rows($resSICONV)>0){
		      echo "<tr><td><strong>Aprovado SICONV:</strong></td><td>".$arraySICONV['Descricao']."</td></tr>";
		}
		if(odbc_num_rows($resApPres)>0){
		      echo "<tr><td><strong>Aprovado Presid&ecirc;ncia:</strong></td><td>".$arrayApPres['Descricao']."</td></tr>";
		}
		echo "</table>";
		echo "</td><td width='20%'></td></tr></table></div><br><br>";
	}
//Função para montar grade de Itens da CI Pendentes

function detalhaItensCi($UserCiItens,$controleCiItens,$idCiItens,$dataCiItens,$solicCiItens,$descCiItens,$valorCiItens,$controleCiItens){
	    include "mb.php";
		require "conectsqlserverciprod.php";
		$SQLCiItensV="Select
  COISOLIC.*,
...
function detalhamentoItensCi($idItensDetalhado,$controleItensDetalhado,$seqIdItensDetal,$UserCiItensDetail, $idCiItensDetail,$dataCiItensDetail,$solicCiItensDetail,$descCiItensDetail,$valorCiItensDetail){
	    include "mb.php";
		require "conectsqlserverci.php";
		$SQLItDiaViag="Select

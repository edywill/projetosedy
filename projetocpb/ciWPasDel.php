...
session_start();
$solicitacao=$_GET['id'];
$sqlRPA=odbc_exec($conCab,"SELECT cd_solicitacao,sequencia,valor FROM TEITEMSOLPASSAGEM(nolock) WHERE id_registro=".$solicitacao."") or die("<p>".odbc_errormsg());
...
$delItem="DELETE FROM TEITEMSOLPASSAGEM WHERE id_registro='".$solicitacao."'";
$resdelItem=odbc_exec($conCab, $delItem) or die("<p>".odbc_errormsg()); 
 if($resdelItem && $updItem){
?>
       <script type="text/javascript">
       alert("Exclu\u00eddo com sucesso");
       window.location="ciWItensExclusivos.php";
       </script>
       <?php
}else{
	?>

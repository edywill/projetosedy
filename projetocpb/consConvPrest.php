...
$resultado = mysql_fetch_array($resultado);
		if (!empty($resultado)){	
			...
			echo "<th>Datas</th>";
			echo "<th colspan='1' align='center'>Prestar Contas</th>";
			echo "</tr>";
			$cadastro = mysql_query($sql);
				while ($linha = mysql_fetch_object($cadastro)) {
					...
					echo "<td><form action='atValConvPrest.php' method='POST' name='editarV'><input name='idEvento' type='hidden' value='".$linha->id."'/><input name='descEvento' type='hidden' value='".utf8_encode ($linha->nome)."'/><input type='submit' class='button' value='Eventos' /></form></td>";
					echo "</tr>";				
				}	
			echo "</table></div>";
		} else {
		...
		if (!empty($resultadoRh)){	
			...
			$cadastroRh = mysql_query($sqlRh);
				while ($linhaRh = mysql_fetch_object($cadastroRh)) {
					...
					echo "<td><form action='atConvPermPrest.php' method='POST' name='editarP'><input name='idEvento' type='hidden' value='".$linhaRh->id."'/><input type='submit' class='button' value='RH Permanente' /></form></td>";
					echo "</tr>";				
				}	
			echo "</table></div>";
		} else {

...
$sqlQueryUpdateOrdem="UPDATE aquiordem SET doc='".utf8_decode($documento)."',fatura='".htmlentities(addslashes($fatura))."', descric='".htmlentities(addslashes($descricao))."', vlunit='".trim($vlunit)."',dtentrega='".utf8_decode($dtentrega)."',emissor='".utf8_decode($emissor)."',evento='".utf8_decode($evento)."',data='".$dataEmissao."' WHERE id='".$idOrdem."'";
...
where Cd_empresa='".$arrayOrdemGeral['cdempres']."'"));
	...
	?>
<p>
Ordem de Serviço/Compra n.º:  <strong><?php echo $_SESSION['idOsImpSession']."/".$_SESSION['anoOsImpSession'];?></strong>
</p>
...
WHERE aquipedidoitem.idos='".$idOrdem."'");
	...
	while($objPedidoOrdem=mysql_fetch_object($sqlPedidosOrdem)){
		...
		echo "<tr><td><font>".utf8_decode($objPedidoOrdem->nome)."</font></td><td align='center'><font>".$objPedidoOrdem->qtd."</font></td><td align='center'><font>".number_format($valorItem,2,",",".")."</font></td><td align='center'><font>".number_format($valorTotalItem,2,",",".")."</font></td></tr>";
		}
	?>
    <tr>

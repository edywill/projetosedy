...
if(empty($qtd)){
	...
	$tipo='';
	if($idatl==0){
	$sqlDuplicidade=mysql_num_rows(mysql_query("SELECT cdmat FROM aquimat WHERE idreg='".$idRegistro."' AND cdmat='".$material."'"));
	if($sqlDuplicidade>0){
		 $valida=1;
					$countError++;
...
if($valida==0){
	if($idatl==0){
	$sqlInsertItemMat=mysql_query("INSERT INTO aquimat(idreg,cdmat,quant,vlunit) VALUES ('".$idRegistro."','".$material."','".$qtd."','".$vlunit."')");
	}else{
		$sqlInsertItemMat=mysql_query("UPDATE aquimat SET idreg='".$idRegistro."',cdmat='".$material."',quant='".$qtd."',vlunit='".$vlunit."' WHERE id='".$idatl."'");
		}

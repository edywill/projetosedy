...
if($_SESSION['solicitacao']<>$_POST['sol']){
		$valida=1;
		$countError++;
		$errorMsg.='Erro['.$countError.']: Voce iniciou uma nova CI neste navegador. Para continuar trabalhando na CI: '.$_POST["sol"].' utilize a op\\u00e7\\u00e3o Alterar CI. O item n\\u00e3oo foi inclu\\u00eddo! Voc\\u00ea sera direcionado para a CI atual: '.$_SESSION["solicitacao"].'';
	   }else{
$solicitacao=$_SESSION['solicitacao'];
...
if ($cdMaterial == "") {
			$valida=1;
			$countError++;
			$errorMsg.='Erro['.$countError.']: Material em branco.\\n';
	}
...
if(empty($pzent)){
			$valida=1;
			$countError++;
			$errorMsg.='Erro['.$countError.']: Informe o prazo de entrega.\\n';
}
...
$resInsertAcompItens = odbc_exec($conCab, $sqlInsertAcompItens) or die("<p>".odbc_errormsg());
	   ...
	   $valida=1;
		$countError++;
		$errorMsg.='Erro['.$countError.']: O texto possui mais de 2000 caracteres.\\n';
	   }
  }
 }
   }
...
<form action="ciWAlteraDF.php" method="post" name="ciWCriar" onSubmit="this.elements['caditem'].disabled=true;"> 
  ...
  <tr><td>
<strong><br/><font color=red>*</font>Conta Gerencial:</strong></td><td><input type="hidden" name="user" value="<?php echo $usuario; ?>"/><input type="hidden" name="solicitacao" value="<?php echo $solicitacao; ?>"/><input type="hidden" name="sequencia" value="<?php echo $sequencia; ?>"/> 
<br/><input  class="input" name="cgeren" id="cgeren" autofocus type="text" size="50"  value="<?php echo $gerenComp; ?>"/>
<input type="hidden" name="endereco" id="endereco" class="input" size="8" value="1"/><br />
<input class="input" name="redcont" id="redcont" type="hidden" size="50"  value="<?php echo $redcontComp; ?>" />

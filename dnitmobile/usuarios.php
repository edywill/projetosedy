...
<table id="example" width="100%" cellpadding='0' cellspacing='0' border='0' class='display' name='tabela2'>
  <thead>
   <tr><td colspan="11" align="right"><font size="+3" color="#000066"><strong>USUÁRIOS CADASTRADOS</strong></font></tr>
			<tr bgcolor="#FFFFFF">
					<th width='5%' height="21">ID</th>
					<th width='35%'>Nome</th>
                    <th width='30%'>E-mail</th>
					<th width='20%'>Dispositivos</th>
			...
			while ($resultado = odbc_fetch_object($query)){
					if(!empty($resultado->email)){
						$nome=$resultado->name;
						$dispositivo='Android';
						if(!empty($resultado->iphone)){
							$dispositivo='Iphone';
							}	
            	echo "<tr><td><font size='-2'>".strtoupper($resultado->id)."</font></td><td><font size='-1'>".utf8_encode($nome)."</font></td><td><font size='-1'>".trim($resultado->email)."</font></td><td><font size='-1'>".$dispositivo."</font></td><td align='center'><form action='editarUser.php' method='post' name='edit'><input type='hidden' name='iduser' value='".$resultado->id."'/><input type='submit' name='edit' value='Editar' style='background-color:#9F9'/></form></td></tr>";
				}
			}
			?>
            </table>
